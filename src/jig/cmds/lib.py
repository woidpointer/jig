...
class Lib:
    ...
    def generate(self, target):
        ...
        project_template = f"{self.template}/cpp/lib"
        run_copy(
            project_template,
            target,
            data={"libname": self.name, "libname_lcase": self.name.lower()},
        )
        # append to CMakeLists.txt
        parent_cmake = f"{target.rstrip('/')}/CMakeLists.txt"
        if Path(parent_cmake).is_file():
            print(f"writing to: {parent_cmake}")
            with open(parent_cmake, "a") as fh:
                fh.write(f"add_subdirectory({self.name})\n")
